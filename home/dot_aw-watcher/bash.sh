...
#
#   Modified by Nick Bradley, Robin Moser
...
function __ash_begin_session() {
  ...
  leading="${current%%ASH=1*}"
  # Preserve any hooks appended after aw-watcher's entry (e.g. ;__zoxide_hook added by zoxide).
  suffix="${current#*ASH=1 __ash_begin_session}"
  suffix="${suffix#;}"    # strip leading semicolon if present
  suffix="${suffix:+;${suffix}}"  # re-add semicolon separator if non-empty

  export PROMPT_COMMAND="${leading}ASH=1 __ash_precmd \${?} \${PIPESTATUS[@]}${suffix}"
...
function __ash_last_command() {
  ...
  local entry="$(HISTTIMEFORMAT="%s " builtin history 1)"
  read -r cmd_no start_ts cmd <<< "${entry//$'\n'/\\n}" # | tr '\n' '⌟')"
...
function __ash_log() {
  ...
  if [ "$ASH_CMD_NUM" != "$no" -a -n "$cmd" ] ; then

      res="$(aw_post_event ${duration} ${start} ${no} ${rval} "${cmd_cwd}" "${cmd}")"
      echo "($no:$rval:$cmd_cwd) $cmd" >> ~/.cmdlog

      if [[ $res != 200 ]]; then
        echo "AW-Watcher Error: $res" > /dev/stderr
      fi
